...
export const TimerWidget: React.FC = () => {
  ...
  const rotaryTicks = useMemo(() => {
    ...
    for (let i = 0; i < totalTicks; i++) {
        ...
        const x1 = cx + (radius - len/2) * Math.cos(angleRad);
        const y1 = cy + (radius - len/2) * Math.sin(angleRad);
        const x2 = cx + (radius + len/2) * Math.cos(angleRad);
        const y2 = cy + (radius + len/2) * Math.sin(angleRad);
            ...
            >
                ...
                <div className="absolute right-4 top-4 bottom-8 w-16 flex flex-col items-center justify-between z-[60] pointer-events-auto">
                    ...
                    <button 
                        ...
                        className="p-3 text-monkey-sub hover:text-monkey-error transition-colors rounded-full bg-[#2c2e31] hover:bg-[#3e4044] border border-monkey-sub/20 mb-4"
                    >
                        <X size={24} />
                    </button>
                    ...
                    <div className="flex flex-col items-center gap-4">
                         <button 
                            ...
                            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all active:scale-95 bg-[#2c2e31] hover:bg-[#3e4044] z-50 ${isRunning ? 'text-monkey-text border border-monkey-sub/50' : 'text-monkey-main border border-monkey-main'}`}
                        ...
                        <button 
                            onClick={handleReset}
                            className="w-10 h-10 rounded-full border border-monkey-sub/30 bg-[#2c2e31] text-monkey-sub flex items-center justify-center hover:border-monkey-main hover:text-monkey-main hover:bg-[#3e4044] transition-all active:scale-90"
                            title="Reset"
                        >
                            <RotateCcw size={18} />
                        </button>
                    ...
                    <div className="flex flex-col gap-2">
                        <button onClick={() => { setMode('STOPWATCH'); setIsRunning(false); }} className={`p-2 rounded-full transition-all border bg-[#2c2e31] ${mode === 'STOPWATCH' ? 'text-monkey-main border-monkey-main' : 'text-monkey-sub border-monkey-sub/20 hover:text-white hover:border-white hover:bg-[#3e4044]'}`} title="Stopwatch"><Watch size={16}/></button>
                        <button onClick={() => { setMode('TIMER'); setIsRunning(false); }} className={`p-2 rounded-full transition-all border bg-[#2c2e31] ${mode === 'TIMER' ? 'text-monkey-main border-monkey-main' : 'text-monkey-sub border-monkey-sub/20 hover:text-white hover:border-white hover:bg-[#3e4044]'}`} title="Timer"><Timer size={16}/></button>
                        <button onClick={() => { setMode('POMODORO'); setIsRunning(false); }} className={`p-2 rounded-full transition-all border bg-[#2c2e31] ${mode === 'POMODORO' ? 'text-monkey-main border-monkey-main' : 'text-monkey-sub border-monkey-sub/20 hover:text-white hover:border-white hover:bg-[#3e4044]'}`} title="Pomodoro"><Coffee size={16}/></button>
                    </div>
                </div>
